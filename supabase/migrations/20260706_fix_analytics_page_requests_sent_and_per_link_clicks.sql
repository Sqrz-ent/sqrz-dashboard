-- Two latent RPC-vs-frontend key mismatches in get_analytics_page:
--   1. Top-level: RPC emitted 'booking_requests_sent' but _app.analytics.tsx
--      reads 'requests_sent' -> "Requests Sent" always showed 0. Rename the key.
--   2. Per-link: RPC emitted 'download_clicks' but the frontend reads link.clicks,
--      AND the count joined on je.visited_via = pbl.link_slug (visited_via holds
--      'sqrz_domain'/'custom_domain', never a slug) so it was always 0. Fix:
--      alias -> 'clicks', join on event_properties->>'link_slug', count both
--      external_link_clicked + legacy download_clicked, excluding destination='page'
--      pill navigations (consistent with the top-level external_link_clicks metric).
