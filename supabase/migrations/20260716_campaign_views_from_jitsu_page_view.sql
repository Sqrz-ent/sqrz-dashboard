-- Campaign "driven views" now come from jitsu page_view (client-side, one per
-- real browser load) instead of profile_views. profile_views is a server-render
-- counter that ~2x over-counts Meta in-app-browser traffic (prefetch renders
-- with no client JS and no working dedup: the sqrz_session cookie is never set
-- server-side, and the UA+IP fingerprint differs between the prefetch and the
-- real load). jitsu page_view matches Meta's own landing-page-view count, so it
-- is the trustworthy source.
--
-- Unique visitors is dropped for campaigns: jitsu page_view is cookieless (no
-- session_id / fingerprint), so there is no per-visitor key to dedupe on. It is
-- returned as NULL + a reason rather than a misleading number (leaving it on
-- profile_views would report unique > views).
--
-- Only the boost_campaigns block changes; everything else is unchanged.
    ...
    'boost_campaigns', (
      ...
      FROM (
        SELECT
          ...
          -- Campaign views: jitsu page_view (client-side, one per real browser
          -- load) — matches Meta's own count. profile_views ~2x over-counts Meta
          -- in-app-browser traffic (server-render prefetch, no working dedup),
          -- so it is intentionally NOT used for campaign views.
          (SELECT COUNT(*) FROM jitsu_events je WHERE je.boost_campaign_id = b.id AND je.event_type = 'page_view') as driven_views,
          -- Unique visitors is not measurable for campaign traffic: jitsu
          -- page_view is cookieless (no session_id / fingerprint). Report NULL +
          -- a reason rather than a misleading number.
          ...
          b.stat_impressions,
          b.stat_reach,
          b.stat_profile_visits,
          b.stat_link_clicks,
          b.stat_cost_per_click,
          b.stat_cpm,
          b.stat_channel_breakdown,
          b.stat_creative_breakdown
